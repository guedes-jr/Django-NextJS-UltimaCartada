...
PUBLIC_ORIGIN="${PUBLIC_ORIGIN:-https://magalyabreu.com.br}"
...
assert_http_status() {
  ...
  if echo "301 302 307 308" | grep -Fq "$status"; then
    fail "$url redirecionou com HTTP $status. Use PUBLIC_ORIGIN com a URL final em HTTPS, por exemplo: PUBLIC_ORIGIN=https://magalyabreu.com.br deploy-cartada"
  fi

  if [ "$status" = "400" ]; then
    fail "$url respondeu HTTP 400. Verifique se '$PUBLIC_HOST' está em ALLOWED_HOSTS no backend/.env."
  fi
